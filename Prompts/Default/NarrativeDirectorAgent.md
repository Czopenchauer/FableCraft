...
Reference the <existing_characters> input data for all NPCs present in the scene and their state. If it's not there 
run get_character_state function calls to retrieve current state of the character including his personality and goals.
...
1. **kg_verification**: Document your KG query results
    ...
    - If KG shows it exists, DO NOT request creation—reuse existing!
...
```json
{
  "writer_instructions": {                                       // Instructions consumed by the Writer agent
  ...
  "creation_requests": {                                         // Content to be created by crafters
  ...
  "narrative_tracking": {                                        // State tracking for persistence and analytics
