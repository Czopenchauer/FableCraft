...
1. **Character Identification**:
    ...
    - Final output must use FULL canonical names (e.g., "[Character Name]", not "[First Name]")
...
- If previous tracker has "[First Name]" but character_list has "[Character Name]", correct it
...
- Example:
  ```json
  [
    {"name": "[Character Name]", "last_known_location": "[Region Name] > [Location Name]"},
    {"name": "[Character Name]", "last_known_location": "[Region Name] > [Location Name]"}
  ]
  ```
...
- Format: `dd/MM/yyyy HH:mm:ss (Time of Day)`
- Use 24-hour clock (00:00:00-23:59:59)
- Time of Day labels:
    - Dawn: 05:00:00-06:59:59
    - Morning: 07:00:00-11:59:59
...
3. **Resolution Rules**:
   ...
   - **No Match Found**:
      ...
      - Infer hierarchy from context (if in "[City Name]", use established city data)
      - Flag as `[NEW]` for potential knowledge graph addition

4. **Hierarchy Inheritance**:

IF entering "the cellar" AND current_location = "[Tavern Name]"
THEN location = "[District Name] > [City Name] > [Tavern Name] > Cellar"
...
- List ALL characters/enemies on scene. For generic NPCs use generic term: "guard", "soldier", etc.
...
- Example: Narrative says "[Character Name]" and character_list contains "[Character Name]" → use directly
...
- This applies to ALL of the following cases:
  ...
  - Aliases or nicknames ("the silver-haired stranger", "Windy")
  - Titles or roles ("the ranger", "Captain", "the blacksmith")
  - Description-based references ("the tall figure with the bow")
...
- If mismatch (e.g., prev has "[First Name]", character_list has "[Character Name]") → correct to canonical form
...
|---------------------|------------------------|--------|--------|
| "[Character Name]" | "[Character Name]" | Use directly | "[Character Name]" |
| "[First Name]" | "[Character Name]" | FETCH | "[Character Name]" |
| "[First Name]" | "[Character Name]" | FETCH | "[Character Name]" |
| "the ranger" | (multiple rangers) | FETCH | (resolved name or flag ambiguous) |
| "the silver-haired stranger" | "[Character Name]" (matches description) | FETCH | "[Character Name]" |
...
|---------------------|-------------------|--------|
| "[Character Name]" | "[Character Name]" | Keep as-is |
| "[First Name]" | "[Character Name]" | Correct to "[Character Name]" |
| "[First Name]" | "[Character Name]" | Correct to "[Character Name]" |
...
2. Is this a partial/shortened version of a name in `character_list`?
   - Yes → CORRECT to the full canonical name (e.g., "[First Name]" → "[Character Name]")
