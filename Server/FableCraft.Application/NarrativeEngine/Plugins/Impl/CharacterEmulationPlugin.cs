...
internal sealed class CharacterEmulationPlugin : PluginBase
{
    ...
    [Description(
        "Emulate a character's action. Use this to generate character dialogue, actions, reactions, or decisions that are consistent with their established traits and the narrative context.")]
    public async Task<string> EmulateCharacterActionAsync(
        ...
        [Description("External events the character can perceive right now. Must NOT contain anything about the character themselves.")]
        string situation,
        [Description("What you need from them. See **Query Types**.")]
        string query
    )
    {
        if (!Context?.LatestTracker()?.Scene?.CharactersPresent.Contains(characterName) ?? true)
        {
        ...
        var result = await _characterAgent.EmulateCharacterAction(situation, query, characterName);
